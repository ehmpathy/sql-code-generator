...
import { getTypeDefinitionFromDeclaration } from '@src/domain.operations/sqlToTypeDefinitions/getTypeDefinitionFromDeclaration';
...
class ErrorExtractingTypeDefinitionFromDeclaration extends Error {
  constructor({
    ...
    const declarationTypeCommonName =
      declaration instanceof QueryDeclaration ? 'query' : 'resource';
    const message = `
...
export const getTypeDefinitionFromDeclarationWithHelpfulError = ({
  ...
  try {
    ...
    const failureMessage = `  ${chalk.bold(chalk.red('x'))} ${chalk.red(
      chalk.bold('[PARSED]'),
    )} ${chalk.bold(declaration.path)}\n`;
    console.log(failureMessage); // tslint:disable-line no-console

    // and pass the error up
    throw new ErrorExtractingTypeDefinitionFromDeclaration({
      declaration,
      error,
    });
  }
