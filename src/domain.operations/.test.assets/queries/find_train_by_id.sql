...
SELECT
  ...
  (
    SELECT COALESCE(array_agg(carriage.uuid ORDER BY carriage_ref.array_order_index), array[]::varchar[]) AS array_agg
    FROM carriage
  ...
  (
    SELECT COALESCE(array_agg(train_engineer.uuid ORDER BY train_engineer_ref.array_order_index), array[]::varchar[]) AS array_agg
    FROM train_engineer
