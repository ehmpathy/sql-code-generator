...
import { extractDataTypeFromColumnOrArgumentDefinitionSql } from '@src/domain.operations/sqlToTypeDefinitions/resource/common/extractDataTypeFromColumnOrArgumentDefinitionSql';
...
*/
export const extractOutputFromFunctionSql = ({
  sql,
}: {
  sql: string;
}): TypeDefinitionOfResourceTable | DataType[] => {
  // 0. strip comments
  ...
  const sqlBetweenReturnsAndNextKeyword = sqlAfterReturns.split(
    /(?:BEGIN|AS|LANGUAGE)/gi,
  )[0]!; // BEGIN is next in mysql, AS or LANGUAGE are next in postgres
  ...
  const dataTypeOrTableDef = (() => {
    const isTableOutput = new RegExp(/^ ?TABLE ?\(/i).test(
      sqlBetweenReturnsAndNextKeyword,
    ); // detect whether this function outputs a table or not
    if (isTableOutput) {
  ...
  if (!dataTypeOrTableDef)
    throw new Error('output type could not be extracted from function sql');
  return dataTypeOrTableDef instanceof TypeDefinitionOfResourceTable
    ? dataTypeOrTableDef
    : [dataTypeOrTableDef]; // table references should be returned exactly. DataType's should be returned as an array
};
