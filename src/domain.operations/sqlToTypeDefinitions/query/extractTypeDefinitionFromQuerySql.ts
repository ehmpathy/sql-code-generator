...
import { TypeDefinitionOfQuery } from '@src/domain.objects/TypeDefinitionOfQuery';
...
export const extractTypeDefinitionFromQuerySql = ({
  name,
  path,
  sql,
}: {
  ...
  const tableReferences = extractTableReferencesFromQuerySql({
    sql: strippedSql,
  });

  // 3. grab the input variable definitions
  const inputVariables = extractInputVariablesFromQuerySql({
    sql: strippedSql,
  });
