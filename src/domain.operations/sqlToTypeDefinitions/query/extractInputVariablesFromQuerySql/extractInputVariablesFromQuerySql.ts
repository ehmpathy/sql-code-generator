...
import { flattenSqlByReferencingAndTokenizingSubqueries } from '@src/domain.operations/sqlToTypeDefinitions/query/common/flattenSqlByReferencingAndTokenizingSubqueries';
...
export const extractInputVariablesFromQuerySql = ({ sql }: { sql: string }) => {
  ...
  const suffixedTokenToTokenMap: {
    ...
    Object.entries(tokenToSuffixedTokensMap).flatMap(
      ([token, suffixedTokens]) => {
