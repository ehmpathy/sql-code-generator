...
export const extractInputVariablesFromQuerySql = ({ sql }: { sql: string }) => {
  ...
  const suffixedTokenSql = Object.keys(tokenToSuffixedTokensMap).reduce((sqlNow, thisToken) => {
    const suffixedTokens = tokenToSuffixedTokensMap[thisToken];
    let sqlToUpdate = sqlNow;
    suffixedTokens.forEach((suffixedToken) => {
      sqlToUpdate = sqlToUpdate.replace(new RegExp(`(?<=[^:])(${thisToken})(?!#)`), suffixedToken);
    });
    return sqlToUpdate;
  }, sql);
  ...
  const definitionsWithSuffixedTokens = suffixedInputVariableTokens.map((token) => {
  ...
  const suffixedTokenToTokenMap: { [index: string]: string } = Object.fromEntries(
  ...
  const definitions = definitionsWithSuffixedTokens.map((definitionWithSuffixedToken) => {
    const tokenForSuffixedToken = suffixedTokenToTokenMap[`:${definitionWithSuffixedToken.name}`].replace(':', '');
    return new TypeDefinitionOfQueryInputVariable({ ...definitionWithSuffixedToken, name: tokenForSuffixedToken });
  });
