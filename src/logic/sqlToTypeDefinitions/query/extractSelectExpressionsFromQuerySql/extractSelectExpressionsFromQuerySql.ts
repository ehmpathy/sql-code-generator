...
export const extractSelectExpressionsFromQuerySql = ({
  ...
  const { flattenedSql, references: subqueries } = flattenSqlByReferencingAndTokenizingSubqueries({ sql: strippedSql });
  ...
  const everythingAfterSelect = (() => {
    ...
    if (partsSplitOnSelect.length > 2) throw new Error('more than one "select" keyword found; unexpected'); // should have been flattened out
  ...
  return selectExpressionsSql.map((thisSql) =>
    extractTypeDefinitionFromSelectExpressionSql({ sql: thisSql, subqueries, inASubquery }),
  );
};
