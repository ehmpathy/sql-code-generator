...
export const extractTypeDefinitionFromTableReference = ({ sql }: { sql: string }) => {
  ...
  const [__, functionName] = tableName ? [, null] : new RegExp(FUNCTION_NAME_MATCHER_REGEX).exec(sql) ?? []; // tslint:disable-line no-unused

  // make sure it matches one of the above
  if (!tableName && !functionName)
    throw new Error('could not identify the referenced table from table reference sql; unexpected'); // fail fast

  // grab the alias, if any
  const [___, specifiedAlias] = new RegExp(SPECIFIED_ALIAS_MATCHER_REGEX).exec(sql) ?? []; // tslint:disable-line no-unused
